...
const FERRY_TYPES = new Set(["ferry"]);
...
export function useTransitLayers(modes: Set<TransitMode>, selectedMetro: string | null) {
  ...
  const filteredRoutes = useMemo(() => {
    ...
    return {
      ...
      features: routeData.features.filter((f) => {
        ...
        if (FERRY_TYPES.has(type ?? "")) return modes.has("ferry");
        if (RAIL_TYPES.has(type ?? "")) return modes.has("rail");
        return modes.has("bus");
      }),
    };
  ...
  const availableModes = useMemo(() => {
    if (!routeData) return { rail: false, bus: false, ferry: false };
    let hasRail = false;
    let hasBus = false;
    let hasFerry = false;
    for (const f of routeData.features) {
      const type = f.properties?.route_type as string | undefined;
      if (FERRY_TYPES.has(type ?? "")) hasFerry = true;
      else if (RAIL_TYPES.has(type ?? "")) hasRail = true;
      else hasBus = true;
      if (hasRail && hasBus && hasFerry) break;
    }
    return { rail: hasRail, bus: hasBus, ferry: hasFerry };
  }, [routeData]);
  ...
  return (
    <>
      ...
      <button
        ...
        title={ferryEnabled ? "Hide ferry" : "Show ferry"}
        aria-label={ferryEnabled ? "Hide ferry routes" : "Show ferry routes"}
        className={`flex items-center gap-1.5 rounded-md px-2 py-1.5 text-xs font-medium transition-colors ${
          ferryEnabled
